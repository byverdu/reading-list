...
const metaReady = fetch(META_URL).then((response) => response.json());

function sortByDateDesc(articles) {
	const toSortable = (date) => date.split('-').reverse().join('-');
	return [...articles].sort(
		(first, second) =>
			new Date(toSortable(second.date)) - new Date(toSortable(first.date)),
	);
}
...
const articlesReady = loadArticles().then((articles) => {
...
class Tags extends HTMLElement {
	...
	sortTags() {
		const counts = window.allArticles.reduce((counts, article) => {
	...
	// Map a count to a heat colour: most frequent → red, least → blue.
	// Log scale so a few dominant tags don't skew everything else to one end.
	static heatColor(count, min, max) {
		const minLog = Math.log(min);
		const maxLog = Math.log(max);
		const ratio =
			maxLog === minLog ? 1 : (Math.log(count) - minLog) / (maxLog - minLog);
		const hue = (1 - ratio) * 240; // ratio=1 → 0 (red), ratio=0 → 240 (blue)
	...
	render() {
		...
		sorted.forEach(([tag, count]) => {
			const button = document.createElement('button', { is: 'tag-item' });

			button.setAttribute('data-tag', tag);
			button.style.setProperty('--heat', Tags.heatColor(count, min, max));
...
class ArticleItem extends HTMLLIElement {
	...
	_render() {
		...
		const article = window.allArticles.find((entry) => entry.id === id);
		...
		if (notes) {
			...
			notesEl.className = 'article-notes';
			notesEl.innerHTML = highlight(notes, query);
			const tagsEl = this.querySelector('.article-tags');
			this.querySelector('.article-main').insertBefore(notesEl, tagsEl);
		}
		...
		for (const tag of tags || []) {
			...
			button.className = active ? 'article-tag active' : 'article-tag';
...
async function getFiltered() {
	...
	return window.allArticles.filter((article) => {
		const matchesTag =
			Tags.activeTags.size === 0 ||
			(article.tags || []).some((tag) => Tags.activeTags.has(tag));
...
function buildArticleSkeletons(total) {
	...
	const rowsToFill = Math.ceil(window.innerHeight / SKELETON_ROW_HEIGHT);
	const count = Math.min(rowsToFill, total);
	...
	list.innerHTML = Array.from(
		...
		<li class="article-skeleton" aria-hidden="true">
			<div class="article-main">
				<div class="skeleton skeleton-title" style="width:${widths[index % widths.length]}"></div>
...
async function init() {
	...
	try {
		...
		document.getElementById('articles').innerHTML =
			`<li class="empty"><strong>Could not load reading list.</strong><span class="error">${error}</span></li>`;
	}
}
